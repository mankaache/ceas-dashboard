...
const DeleteArticle = dynamic(
  () => import("@/components/pages/articles").then((mod) => mod.DeleteArticle),
  { ssr: false }
);

export const ArticleList: React.FC<{
  onEditClick: (article?: DocumentSnapshot<IArticle>) => void;
  onDeleteClick: (article: DocumentSnapshot<IArticle>) => void;
}> = ({ onDeleteClick, onEditClick }) => {
  const articlesQuery = query(
    collection(firestore, "articles"),
    orderBy("modifiedAt", "desc")
  );
  const [value, loading, error] = useCollection(articlesQuery, {
      ...
      <Card className="w-full fade-enter fade-enter-active">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="title">
            ...
            <CardDescription>Gérer et afficher vos articles</CardDescription>
          </div>
        ...
        {!Boolean(value?.docs.length) ? (
          <div className="mx-auto my-4 w-full flex items-center justify-center">
            <p className="text-3xl p-4">Pas des articles</p>
          </div>
        ) : (
                  ...
                  >
                    <TableCell className="hidden sm:table-cell">
                      <Image
                        ...
                        height="64"
                        alt={article.data().image.caption}
                        width="64"
                      />
                    </TableCell>
                    ...
                    <TableCell className="hidden md:table-cell min-w-[180px] max-w-sm">
                      {dayjs(article.data().modifiedAt).format(
