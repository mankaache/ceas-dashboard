...
const DeleteAlumini = dynamic(
  () =>
    import("@/components/pages/alumini").then((mod) => mod.DeleteAlumini ),
  { ssr: false }
);

export const AluminiList: React.FC<{
  onEditClick: (alumini?: DocumentSnapshot<IAlumini>) => void;
  onDeleteClick: (alumini: DocumentSnapshot<IAlumini>) => void;
      ...
      <Card className="w-full fade-enter fade-enter-active">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="title">
            ...
            <CardDescription>Gérer et afficher vos Alumni</CardDescription>
          </div>
        ...
        {!Boolean(value?.docs.length) ? (
          <div className="mx-auto my-4 w-full flex items-center justify-center">
            <p className="text-3xl p-4">Pas d'alumni</p>
          </div>
        ) : (
              ...
              <TableBody>
                {value?.docs.map((alumini, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="font-medium min-w-[250px] max-w-sm">
                      {alumini.data().description}
                    </TableCell>
                    <TableCell className="font-medium min-w-[150px] max-w-sm">
                      {alumini.data().category}
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={cn(
                            alumini.data().status == "active"
                    ...
                    <TableCell className="hidden md:table-cell min-w-[180px] max-w-sm">
                      {dayjs(alumini.data().modifiedAt).format(
