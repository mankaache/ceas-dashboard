...
const DeleteEvent = dynamic(
  () => import("@/components/pages/events").then((mod) => mod.DeleteEvent),
  { ssr: false }
);

export const EventList: React.FC<{
  onEditClick: (event?: DocumentSnapshot<IEvent>) => void;
  onDeleteClick: (event: DocumentSnapshot<IEvent>) => void;
      ...
      <Card className="w-full fade-enter fade-enter-active">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="title">
            ...
            <CardDescription>Gérer et afficher vos événements</CardDescription>
          </div>
        ...
        {!Boolean(value?.docs.length) ? (
          <div className="mx-auto my-4 w-full flex items-center justify-center">
            <p className="text-3xl p-4">Pas des événements</p>
          </div>
        ) : (
                ...
                {value?.docs.map((event, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="hidden md:table-cell min-w-[180px] max-w-sm">
                      {dayjs(event.data().modifiedAt).format(
