...
// TrainingProgram Form Component
export const TrainingProgramForm: React.FC<{
  trainingProgram: QueryDocumentSnapshot | null;
  onCancel: () => void;
  onSave: () => void;
}> = ({ trainingProgram, onCancel, onSave }) => {
  const [activeTagIndex, setActiveTagIndex] = React.useState<number | null>(
    null
  );
  const [tags, setTags] = React.useState<ITag[]>(
    trainingProgram?.data().tags ?? []
  );
    ...
    defaultValues: trainingProgram
      ? {
          image: {
            ...
            caption: trainingProgram.data().image.caption,
          },
          title: trainingProgram.data().title,
          excerpt: trainingProgram.data().excerpt,
          slug: trainingProgram.data().slug,
          content: trainingProgram.data().content,
          date: trainingProgram.data().date,
          location: trainingProgram.data().location,
          tags: trainingProgram.data().tags || [],
          // author: trainingProgram.data().author,
          category: trainingProgram.data().category,
        }
      : { tags: [] },
  ...
  const handleTitleChange = (
    trainingProgram: React.ChangeEvent<HTMLInputElement>
  ) => {
    const title = trainingProgram.target.value;
  ...
  const onSubmitAdd = async (data: z.infer<typeof trainingProgramSchema>) => {
    // console.log(data);
    // console.log(files);
    ...
    if (Boolean(files.length)) {
      try {
        ...
        if (data.custom_category) {
          await addSubcategory("training-programs", {
        ...
        // const trainingProgramRef = doc(collection(firestore, "trainingPrograms"))
        const trainingProgramRef = collection(firestore, "training-programs");
        const trainingProgramData = {
          // id: trainingProgramRef.id,
          image: {
            src: url,
        ...
        // await setDoc(trainingProgramRef, trainingProgramData)
        await addDoc(trainingProgramRef, trainingProgramData);
  ...
  const onSubmitEdit = async (data: z.infer<typeof trainingProgramSchema>) => {
    if (!(trainingProgram || Boolean(files.length))) {
      toast.error("No file present!");
    } else {
      try {
        Swal.fire("S'il vous plaît, attendez...");
        ...
        const url = !Boolean(files.length)
          ? trainingProgram?.data().image.src
          : // @ts-ignore
        ...
        const trainingProgramData = {
          ...
          location: data.location,
          category: data.category ?? data.custom_category,
          createdAt: trainingProgram?.data().createdAt,
          modifiedAt: dayjs().toISOString(),
          status: "active",
        };

        await setDoc(trainingProgramRef, trainingProgramData, { merge: true });
  ...
  return (
    ...
    >
      <Card className="w-full fade-enter fade-enter-active">
        <CardHeader>
          <CardTitle>
            {trainingProgram ? "Modifier" : "Ajouter un"} programme de formation
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form
              ...
              onSubmit={form.handleSubmit(
                // @ts-ignore
                trainingProgram ? onSubmitEdit : onSubmitAdd
              )}
              className="space-y-8"
            >
              <FormField
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar
                              ...
                              onSelect={(date) => {
                                form.setValue("date", dayjs().toISOString());
